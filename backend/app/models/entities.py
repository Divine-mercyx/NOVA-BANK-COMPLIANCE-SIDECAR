...
class DtdTransaction(Base):
    ...
    receiver_name: Mapped[str] = mapped_column(String(255))
    receiver_account: Mapped[str] = mapped_column(String(50))
    source_institution_code: Mapped[str | None] = mapped_column(String(20), nullable=True)
    source_institution_name: Mapped[str | None] = mapped_column(String(255), nullable=True)
    dest_institution_code: Mapped[str | None] = mapped_column(String(20), nullable=True)
    dest_institution_name: Mapped[str | None] = mapped_column(String(255), nullable=True)
    branch_code: Mapped[str] = mapped_column(String(20), default="001")
    narration: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
